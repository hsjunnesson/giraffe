...
void initialize(foundation::Allocator &allocator) {
...

	{
		...
		r = script_engine->RegisterObjectType("vec2", sizeof(glm::vec2), asOBJ_VALUE | asOBJ_POD | asOBJ_APP_CLASS_CAK | asOBJ_APP_CLASS_ALLFLOATS); assert(r >= 0);
		...
		r = script_engine->RegisterObjectType("vec3", sizeof(glm::vec3), asOBJ_VALUE | asOBJ_POD | asOBJ_APP_CLASS_CAK | asOBJ_APP_CLASS_ALLFLOATS); assert(r >= 0);
		r = script_engine->RegisterObjectProperty("vec3", "float x", asOFFSET(glm::vec3, x)); assert(r >= 0);
		r = script_engine->RegisterObjectProperty("vec3", "float y", asOFFSET(glm::vec3, y)); assert(r >= 0);
		r = script_engine->RegisterObjectProperty("vec3", "float z", asOFFSET(glm::vec3, z)); assert(r >= 0);
		r = script_engine->RegisterObjectBehaviour("vec3", asBEHAVE_CONSTRUCT, "void f()", asFUNCTION(vec3_default), asCALL_CDECL_OBJLAST); assert(r >= 0);
		r = script_engine->RegisterObjectBehaviour("vec3", asBEHAVE_CONSTRUCT, "void f(float, float, float)", asFUNCTION(vec3_construct), asCALL_CDECL_OBJLAST); assert(r >= 0);

		r = script_engine->RegisterObjectType("mat4", sizeof(glm::mat4), asOBJ_VALUE | asOBJ_POD | asOBJ_APP_CLASS_CAK); assert(r >= 0);
		r = script_engine->RegisterObjectBehaviour("mat4", asBEHAVE_CONSTRUCT, "void f()", asFUNCTION(mat4_default), asCALL_CDECL_OBJLAST); assert(r >= 0);
...

    {
		...
		r = script_engine->RegisterObjectType("Matrix4f", sizeof(math::Matrix4f), asOBJ_VALUE | asOBJ_POD | asGetTypeTraits<math::Matrix4f>()); assert(r >= 0);
...

    {
        ...
        r = script_engine->SetDefaultNamespace("engine::color"); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f black", const_cast<math::Color4f *>(&engine::color::black)); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f white", const_cast<math::Color4f *>(&engine::color::white)); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f red", const_cast<math::Color4f *>(&engine::color::red)); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f green", const_cast<math::Color4f *>(&engine::color::green)); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f blue", const_cast<math::Color4f *>(&engine::color::blue)); assert(r >= 0);
        r = script_engine->SetDefaultNamespace(""); assert(r >= 0);

        r = script_engine->SetDefaultNamespace("engine::color::pico8"); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f black", const_cast<math::Color4f *>(&engine::color::pico8::black)); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f dark_blue", const_cast<math::Color4f *>(&engine::color::pico8::dark_blue)); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f dark_purple", const_cast<math::Color4f *>(&engine::color::pico8::dark_purple)); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f dark_green", const_cast<math::Color4f *>(&engine::color::pico8::dark_green)); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f brown", const_cast<math::Color4f *>(&engine::color::pico8::brown)); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f dark_gray", const_cast<math::Color4f *>(&engine::color::pico8::dark_gray)); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f light_gray", const_cast<math::Color4f *>(&engine::color::pico8::light_gray)); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f red", const_cast<math::Color4f *>(&engine::color::pico8::red)); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f orange", const_cast<math::Color4f *>(&engine::color::pico8::orange)); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f yellow", const_cast<math::Color4f *>(&engine::color::pico8::yellow)); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f green", const_cast<math::Color4f *>(&engine::color::pico8::green)); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f blue", const_cast<math::Color4f *>(&engine::color::pico8::blue)); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f indigo", const_cast<math::Color4f *>(&engine::color::pico8::indigo)); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f pink", const_cast<math::Color4f *>(&engine::color::pico8::pink)); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("const math::Color4f peach", const_cast<math::Color4f *>(&engine::color::pico8::peach)); assert(r >= 0);
        r = script_engine->SetDefaultNamespace(""); assert(r >= 0);
    }

	{
		...
		r = script_engine->RegisterObjectType("Engine", 0, asOBJ_REF | asOBJ_NOCOUNT); assert(r >= 0);
        r = script_engine->RegisterObjectProperty("Engine", "math::Rect window_rect", asOFFSET(engine::Engine, window_rect)); assert(r >= 0);
		...
		r = script_engine->RegisterGlobalFunction("void transform_sprite(engine::Sprites@ sprites, uint64 sprite_id, math::Matrix4f)", asFUNCTION(transform_sprite_wrapper), asCALL_CDECL); assert(r >= 0);
		r = script_engine->RegisterGlobalFunction("void update_sprites(engine::Sprites@ sprites, float t, float dt)", asFUNCTION(update_sprites_wrapper), asCALL_CDECL); assert(r >= 0);
		r = script_engine->RegisterGlobalFunction("void commit_sprites(engine::Sprites@ sprites)", asFUNCTION(commit_sprites_wrapper), asCALL_CDECL); assert(r >= 0);
		r = script_engine->RegisterGlobalFunction("void render_sprites(engine::Engine@ engine, engine::Sprites@ sprites)", asFUNCTION(render_sprites_wrapper), asCALL_CDECL); assert(r >= 0);
...

    {
        ...
        r = script_engine->RegisterObjectType("rnd_pcg_t", 0, asOBJ_REF | asOBJ_NOCOUNT); assert(r >= 0);
        r = script_engine->RegisterGlobalFunction("void rnd_pcg_seed(rnd_pcg_t@, uint32)", asFUNCTION(rnd_pcg_seed), asCALL_CDECL); assert(r >= 0);
        r = script_engine->RegisterGlobalFunction("float rnd_pcg_nextf(rnd_pcg_t@)", asFUNCTION(rnd_pcg_nextf), asCALL_CDECL); assert(r >= 0);
        r = script_engine->RegisterGlobalProperty("rnd_pcg_t RANDOM_DEVICE", &random_device); assert(r >= 0);
    }
